...
if(isset($_POST['edit'])) {
    ...
    if($_FILES['foto_baru']['error'] === 4) {
        ...
        if(isset($_FILES['foto_baru'])) {
            ...
            $ambil_ektensi = pathinfo($nama_file, PATHINFO_EXTENSION);
            $ektensi_diizinkan = ['jpg', 'jpeg', 'png'];
    ...
    if($_SERVER['REQUEST_METHOD'] == "POST") {
        ...
        if($_FILES['foto_baru']['error'] != 4) {
            if(!in_array(strtolower($ambil_ektensi), $ektensi_diizinkan)){
        ...
        if(!empty($pesan_kesalahan)){
            ...
            $pegawai = mysqli_query($connection, "UPDATE pegawai SET nama = '$nama', jenis_kelamin = '$jenis_kelamin', alamat = '$alamat', no_handphone = '$no_handphone', jabatan = '$jabatan', lokasi_presensi = '$lokasi_presensi', foto = '$nama_file' WHERE id = $id");
            
            $user = mysqli_query($connection, "UPDATE users SET username = '$username', password = '$password', status = '$status', role = '$role' WHERE id_pegawai = $id");
